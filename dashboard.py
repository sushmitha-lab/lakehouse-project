...
<style>
    ...
    .section-title {
        font-size: 13px; letter-spacing: 3px; color: #4a7fa5;
...
def get_weather():
    results = []
    for city in CITIES:
        r = requests.get(f"http://wttr.in/{city}?format=j1")
        c = r.json()["current_condition"][0]
        results.append({
            "city": city,
            "temperature_f": float(c["temp_F"]),
            "humidity": float(c["humidity"]),
            "weather": c["weatherDesc"][0]["value"],
            "wind_speed": float(c["windspeedMiles"]),
        })
    return pd.DataFrame(results)
...
def get_news():
    ...
    for topic in TOPICS:
        r = requests.get(
            ...
            params={"q":topic,"apiKey":NEWS_KEY,"language":"en","pageSize":3,"sortBy":"publishedAt"}
        )
...
with col1:
    ...
    fig.add_trace(go.Bar(
        x=stocks["ticker"], y=stocks["price"],
        marker_color=colors,
        text=[f"${p:.2f}" for p in stocks["price"]],
...
with col2:
    ...
    fig2.add_trace(go.Bar(
        x=stocks["ticker"], y=stocks["change_percent"],
        marker_color=colors,
        text=[f"{p:+.2f}%" for p in stocks["change_percent"]],
        textposition="outside",
        textfont=dict(color="#e8f4ff", size=12)
    ))
...
with col3:
    ...
    fig3.add_trace(go.Bar(
        x=weather["city"], y=weather["temperature_f"],
        marker_color="#4f8ef7",
        text=[f"{t:.0f}°F" for t in weather["temperature_f"]],
        textposition="outside", textfont=dict(color="#e8f4ff", size=11)
    ))
...
with col4:
    ...
    fig4.add_trace(go.Bar(
        x=weather["city"], y=weather["humidity"],
        marker_color="#a78bfa",
        text=[f"{h:.0f}%" for h in weather["humidity"]],
        textposition="outside", textfont=dict(color="#e8f4ff", size=11)
    ))
...
with col5:
    ...
    fig5.add_trace(go.Bar(
        x=weather["city"], y=weather["wind_speed"],
        marker_color="#f6c90e",
        text=[f"{w:.0f} mph" for w in weather["wind_speed"]],
        textposition="outside", textfont=dict(color="#e8f4ff", size=11)
    ))
...
with col6:
    ...
    fig6 = go.Figure(go.Pie(
        labels=news_count["topic"], values=news_count["count"], hole=0.65,
        marker=dict(colors=["#00e5a0","#4f8ef7","#ff4d6d","#f6c90e","#a78bfa"],
                   line=dict(color="#080c14", width=2))
    ))
    fig6.update_layout(
        paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#7a9ab5", size=12),
...
with col7:
    st.dataframe(
        news[["topic","title","source","published_at"]].head(8),
